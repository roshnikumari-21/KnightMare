// Main function with all rules implemented
    ...
    switch(piece_name) {
        ...
        case "king":
            ...
            for (const [dx, dy] of kingMoves) {
                ...
                if (isOnBoard(newX, newY)) {
                    if (!isAlly(newX, newY)) {
                        if (!checkForChecks || isSquareSafe(newX, newY)) {
                            addMoveIfLegal(newX, newY);
                        }
                    }
                }
            ...
            // Castling
            if (checkForChecks && !hasMoved && isSquareSafe(x, y)) {
                const castleSide = {
                    ...
                    if (castlingRights[piece_color][side]) {
                        ...
                        if (rookSquare.piece?.type === 'rook' && !rookSquare.piece.hasMoved) {
                            ...
                            const pathSafe = data.squaresBetween.every(col => 
                                isSquareSafe(x, col) && isEmpty(x, col)
                            );
...
function checkGameState(board, color, castlingRights, enPassantTarget) {
